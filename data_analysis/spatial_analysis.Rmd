...
scp -r hslyman@tadpole.genomecenter.ucdavis.edu:/share/workshop/spatial_workshop/data_analysis/data .
```
...
sample.ident <- sapply(strsplit(Cells(experiment.merged), split = "-"), "[[", 2)
slide <- design[sample.ident, "slide"]
capture.area <- design[sample.ident, "capture.area"]
experiment.merged <- AddMetaData(experiment.merged,
                                 metadata = sample.ident,
                                 col.name = "sample.ident")
experiment.merged <- AddMetaData(experiment.merged,
                                 metadata = slide,
                                 col.name = "slide")
experiment.merged <- AddMetaData(experiment.merged,
...
rm(project.name, experiment.slices, sample.ident, slide, capture.area)
experiment.merged
```
...
SpatialFeaturePlot(experiment.merged, "percent.mito", images = c("Posterior1", "Posterior2"))
```
