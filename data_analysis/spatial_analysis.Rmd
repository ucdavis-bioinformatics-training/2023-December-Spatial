...
Users are reporting issues with Seurat5's FindSpatiallVariableFeatures() function, so we will be reverting to Seurat version 4 for this section of the analysis. In order to avoid managing two installations of Seurat in the same session, we will run Seurat 4 on tadpole.

```{r, SpatiallyVariable.1}
...
# save object
saveRDS(anterior1.cluster3, file = "anterior1_cluster3.rds")
```

Transfer the saved object to the tadpole. Don't forget to change "username" to your username. You may need to paste the command below into a new terminal.
```{bash, eval = FALSE}
scp anterior1_cluster3.rds username@tadpole.genomecenter.ucdavis.edu:/share/workshop/Spatial_Workshop/username/
```

Log into the cluster on the command line, and run the following:
```{bash, eval = FALSE}
...
This launches an R session on the command line. Notice that the prompt has become the ">" character, rather than the "$" associated with bash.
```{r, eval=FALSE}
.libPaths("/share/workshop/Spatial_Workshop/Software")
library("Seurat")
anterior1.cluster3 <- readRDS("anterior1_cluster3.rds")
FindSpatiallyVariableFeatures(anterior1.cluster3)
```
...
lapply(experiment.slices, function(slice){
  lapply(SpatiallyVariableFeatures(slice)[1:5], function(feature){
    SpatialFeaturePlot(slice, features = feature)
  })
})

experiment.slices[[1]]@assays$SCT@meta.features[!(is.na(experiment.slices[[1]]@assays$SCT@meta.features$markvariogram.spatially.variable.rank)),]


example <- readRDS("Anterior_1_spatiallyVariableFeatures.rds")
example <- FindNeighbors(example)
example <- FindClusters(example)
DimPlot(example, reduction = "umap")
SpatialDimPlot(example)
example.cluster5 <- subset(example, seurat_clusters == "5")
DefaultAssay(example.cluster5) <- "Spatial"
example.cluster5[["SCT"]] <- NULL
example.cluster5[["predictions"]] <- NULL
str(example.cluster5@assays)
example.cluster5 <- SCTransform(example.cluster5,
                                assay = "Spatial")
str(example.cluster5@assays)
example.cluster5 <- RunPCA(example.cluster5)
example.cluster5 <- RunUMAP(example.cluster5, dims = 1:30)
example.cluster5 <- FindSpatiallyVariableFeatures(example.cluster5,
                                                  assay = "SCT",
                                                  features = VariableFeatures(example.cluster5),
                                                  selection.method = "markvariogram")
SpatialFeaturePlot(example.cluster5,
                          features = SpatiallyVariableFeatures(example.cluster5)[1])
SpatialFeaturePlot(example.cluster5,
                   slot = "data",
                   features = SpatiallyVariableFeatures(example.cluster5)[2])
```
